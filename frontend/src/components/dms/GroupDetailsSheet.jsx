...
const GroupVerifiedMark = () => (
  <span className="dm-group-member-verified" aria-label="Verified" title="Verified">
        ...
        <div className="dm-group-member-list">
          {visibleMembers.map((member) => {
            ...
            return <div className="dm-group-member" key={member.user_id}>
              ...
              <span className="dm-group-member-name"><strong><span>{member.display_name || member.username}{isMe ? " (you)" : ""}</span>{Number(member.is_verified) === 1 && <GroupVerifiedMark />}</strong><small>@{member.username} · Joined {formatDate(member.joined_at)}</small>{activityLabel && <em>{activityLabel}</em>}</span>

...
import createVineAnalyticsRouter from "./vineAnalyticsRoutes.js";
import createVineCommunityLibraryRouter from "./vineCommunityLibraryRoutes.js";
import createVineCommunityAssignmentsRouter from "./vineCommunityAssignmentsRoutes.js";
import createVineCommunityAttendanceRouter from "./vineCommunityAttendanceRoutes.js";
...
let cleanupExpiredReadNotificationsImpl = null;
const cleanupExpiredReadNotifications = async () => {
  if (!cleanupExpiredReadNotificationsImpl) {
    cleanupExpiredReadNotificationsImpl = createCleanupExpiredReadNotifications({
      db,
      clearVineReadCache,
      readNotificationRetentionDays: VINE_READ_NOTIFICATION_RETENTION_DAYS,
    });
  }
  return cleanupExpiredReadNotificationsImpl();
};
...
let vineCommunityDiscoveryRouter = null;
router.use((req, res, next) => {
  if (!vineCommunityDiscoveryRouter) {
    vineCommunityDiscoveryRouter = createVineCommunityDiscoveryRouter({
      db,
      authenticate,
      authOptional,
  ...
  return vineCommunityDiscoveryRouter(req, res, next);
});
...
let vineCommunitySettingsRouter = null;
router.use((req, res, next) => {
...
let vineCommunityAttendanceRouter = null;
router.use((req, res, next) => {
  if (!vineCommunityAttendanceRouter) {
    vineCommunityAttendanceRouter = createVineCommunityAttendanceRouter({
      db,
      authenticate,
      ensureCommunitySchema,
      ...
      vineCacheTtls: VINE_CACHE_TTLS,
      clearVineReadCache,
    });
  }
  return vineCommunityAttendanceRouter(req, res, next);
});
...
let vineCommunityLibraryRouter = null;
router.use((req, res, next) => {
  if (!vineCommunityLibraryRouter) {
    vineCommunityLibraryRouter = createVineCommunityLibraryRouter({
      db,
      authenticate,
...
router.use((req, res, next) => {
  if (!vineCommunityAssignmentsRouter) {
    vineCommunityAssignmentsRouter = createVineCommunityAssignmentsRouter({
      db,
      authenticate,
      uploadPostCloudinary,
      ensureCommunitySchema,
  ...
  return vineCommunityAssignmentsRouter(req, res, next);
});
...
let vineCommunityInsightRouter = null;
router.use((req, res, next) => {
  if (!vineCommunityInsightRouter) {
    vineCommunityInsightRouter = createVineCommunityInsightRouter({
      db,
      authenticate,
      ensureCommunitySchema,
      getCommunityRole,
  ...
  return vineCommunityInsightRouter(req, res, next);
});
...
let vineAnalyticsRouter = null;
router.use((req, res, next) => {
  if (!vineAnalyticsRouter) {
    vineAnalyticsRouter = createVineAnalyticsRouter({
      db,
      authenticate,
      isModeratorAccount,
      ...
      vineSlowQueryMs: VINE_SLOW_QUERY_MS,
    });
  }
  return vineAnalyticsRouter(req, res, next);
});
...
let vineNotificationRouter = null;
router.use((req, res, next) => {
  if (!vineNotificationRouter) {
    vineNotificationRouter = createVineNotificationRouter({
      db,
      authenticate,
  ...
  return vineNotificationRouter(req, res, next);
});
